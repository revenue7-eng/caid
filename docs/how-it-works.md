...
**The protocol** is the [Assessment Standard](../docs/standard.html), which lists the mandatory requirements for a run. Until every one of them is met, the result stays a private measurement rather than a CAID one. Requirements of that kind are flagged as they come up below.

**The reference run** is the first public measurement, May 2026: 26 models across five providers. The examples below refer to it. Current per-model numbers are on the [Report Cards](../docs/evidence.html).
...
Open [`PROTOCOL.md`](https://github.com/revenue7-eng/caid-benchmark/blob/main/PROTOCOL.md) and the code and the same things carry different names.
